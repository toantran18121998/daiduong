...
function language_school_breadcrumbs() {
	global $post;
	
	
	$homeLink = esc_url(home_url('/'));
	
	$homeText = esc_attr__('Home', 'language-school');
	...
	if (CMSMASTERS_LEARNPRESS && learn_press_is_courses()) {
		...
		echo '<span>' . esc_html__('Search results for', 'language-school') . ": '" . esc_html(get_search_query()) . "'</span>";
		...
		if (is_tax('post_format', 'post-format-gallery')) {
			echo '<span>' . _x('Galleries', 'post format archive title', 'language-school') . '</span>';
		} elseif (is_tax('post_format', 'post-format-image')) {
			echo '<span>' . _x('Images', 'post format archive title', 'language-school') . '</span>';
		} elseif (is_tax('post_format', 'post-format-video')) {
			echo '<span>' . _x('Videos', 'post format archive title', 'language-school') . '</span>';
		} elseif (is_tax('post_format', 'post-format-audio')) {
			echo '<span>' . _x('Audio', 'post format archive title', 'language-school') . '</span>';
		}
		...
		echo '<span>' . esc_html__('No breadcrumbs', 'language-school') . '</span>';
	}
}

		...
		<div class="cmsmasters_single_event_header clearfix">
				...
				'<div class="cmsmasters_single_event_header_left_inner">';
					...
					echo '<div class="tribe-events-schedule updated published clearfix">' . 
						...
						if (tribe_address_exists()) {
							echo '<div class="tribe-events-address cmsmasters_theme_icon_user_address">' . 
			...
			<div class="cmsmasters_single_event_header_right clearfix">
				<div class="tribe-events-back">
					<a href="<?php echo esc_url( tribe_get_events_link() ); ?>"> <?php printf( '&laquo;' . esc_html__( 'All %s', 'language-school' ), $events_label_plural ); ?></a>
				</div>
			...
			echo '<div class="cmsmasters_single_event_inner">';
				...
				if (has_post_thumbnail()) {
					echo '<div class="cmsmasters_single_event_img' . (!tribe_embed_google_map() ? ' cmsmasters_single_event_full_width' : '') . '">' . 
						tribe_event_featured_image($event_id, (!tribe_embed_google_map() ? 'cmsmasters-full-thumb' : 'cmsmasters-event-thumb'), false) . 
					'</div>';
				}
	...
	if ($published_events > 1) {
		echo '<aside id="tribe-events-sub-nav" class="post_nav">';
			if (tribe_get_prev_event_link()) {
				echo '<span class="tribe-events-nav-previous cmsmasters_prev_post cmsmasters_theme_icon_slide_' . (!is_rtl() ? 'prev' : 'next') . '">'; 
			...
			if (tribe_get_next_event_link()) {
				echo '<span class="tribe-events-nav-next cmsmasters_next_post cmsmasters_theme_icon_slide_' . (!is_rtl() ? 'next' : 'prev') . '">';

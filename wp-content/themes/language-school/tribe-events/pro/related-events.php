...
<h2 class="tribe-events-related-events-title"><?php printf( esc_html__( 'Related %s', 'language-school' ), tribe_get_event_label_plural() ); ?></h2>
	...
	<li>
		<div class="tribe-related-events-thumbnail">
			<a href="<?php echo esc_url( tribe_get_event_link( $post ) ); ?>" class="url" rel="bookmark">
				<?php echo (has_post_thumbnail($post->ID)) ? get_the_post_thumbnail($post->ID, 'cmsmasters-event-thumb') : '<span class="cmsmasters_events_img_placeholder cmsmasters_theme_icon_date"></span>'; ?>
			</a>
		</div>
		<div class="tribe-related-event-info">
			<h6 class="tribe-related-events-title summary"><a href="<?php echo esc_url(tribe_get_event_link( $post )); ?>" class="url" rel="bookmark"><?php echo esc_html(get_the_title( $post->ID )); ?></a></h6>
			<?php

...
use MailPoetVendor\PhpConsole\Connector;
use MailPoetVendor\PhpConsole\Handler;
use MailPoetVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \MailPoetVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \MailPoetVendor\PhpConsole\Connector $connector = null, $level = \MailPoetVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('MailPoetVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\MailPoetVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \MailPoetVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \MailPoetVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\MailPoetVendor\PhpConsole\Helper::isRegistered()) {
            \MailPoetVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \MailPoetVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \MailPoetVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \MailPoetVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \MailPoetVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}

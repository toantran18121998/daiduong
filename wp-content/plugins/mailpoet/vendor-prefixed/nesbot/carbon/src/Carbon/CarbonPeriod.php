...
class CarbonPeriod implements \Iterator, \Countable
{
    ...
    {
        return \is_string($callable) && \substr($callable, 0, 2) === 'is' && (\method_exists('MailPoetVendor\\Carbon\\Carbon', $callable) || \MailPoetVendor\Carbon\Carbon::hasMacro($callable));
    }
    /**
    ...
    protected static function parseIso8601($iso)
    {
        ...
        foreach (\explode('/', $iso) as $key => $part) {
            ...
            } elseif ($interval === null && ($parsed = \MailPoetVendor\Carbon\CarbonInterval::make($part))) {
                $interval = $part;
            } elseif ($start === null && ($parsed = \MailPoetVendor\Carbon\Carbon::make($part))) {
                $start = $part;
            } elseif ($end === null && ($parsed = \MailPoetVendor\Carbon\Carbon::make(static::addMissingParts($start, $part)))) {
                $end = $part;
            } else {
    ...
    public function __construct()
    {
        ...
        foreach ($arguments as $argument) {
            if ($this->dateInterval === null && ($parsed = \MailPoetVendor\Carbon\CarbonInterval::make($argument))) {
                $this->setDateInterval($parsed);
            } elseif ($this->startDate === null && ($parsed = \MailPoetVendor\Carbon\Carbon::make($argument))) {
                $this->setStartDate($parsed);
            } elseif ($this->endDate === null && ($parsed = \MailPoetVendor\Carbon\Carbon::make($argument))) {
        ...
        if ($this->startDate === null) {
            $this->setStartDate(\MailPoetVendor\Carbon\Carbon::now());
        }
        if ($this->dateInterval === null) {
            $this->setDateInterval(\MailPoetVendor\Carbon\CarbonInterval::day());
    ...
    {
        if (!($interval = \MailPoetVendor\Carbon\CarbonInterval::make($interval))) {
    ...
    {
        if (!($date = \MailPoetVendor\Carbon\Carbon::make($date))) {
    ...
    {
        if (!\is_null($date) && !($date = \MailPoetVendor\Carbon\Carbon::make($date))) {
     ...
     * @return Carbon
     */
    protected function prepareForReturn(\MailPoetVendor\Carbon\Carbon $date)
    {
        $date = $date->copy();
        if ($this->timezone) {
    ...
    {
        $translator = \MailPoetVendor\Carbon\Carbon::getTranslator();
        $parts = array();
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            ...
            case 'sinceNow':
                return $this->setStartDate(new \MailPoetVendor\Carbon\Carbon(), $first);
            case 'end':
            case 'until':
                return $this->setEndDate($first, $second);
            case 'untilNow':
                return $this->setEndDate(new \MailPoetVendor\Carbon\Carbon(), $first);
            case 'dates':
            case 'between':
            ...
            case 'second':
                return $this->setDateInterval(\call_user_func(
                    ...
                    array($this->isDefaultInterval ? new \MailPoetVendor\Carbon\CarbonInterval('PT0S') : $this->dateInterval, $method),

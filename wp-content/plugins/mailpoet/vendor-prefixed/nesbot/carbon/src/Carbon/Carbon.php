...
class Carbon extends \DateTime implements \JsonSerializable
{
     ...
     * @var string
     */
    const RFC7231_FORMAT = 'MailPoetVendor\\D, d M Y H:i:s \\G\\M\\T';
    /**
     ...
     * @var bool
     */
    protected static $microsecondsFallback = \true;
    /**
     ...
     */
    public static function useMicrosecondsFallback($microsecondsFallback = \true)
    {
     ...
     * @return bool
     */
    public static function compareYearWithMonth($compareYearWithMonth = \true)
    {
    ...
    public static function createSafe($year = null, $month = null, $day = null, $hour = null, $minute = null, $second = null, $tz = null)
    {
        ...
        foreach ($fields as $field => $range) {
            if (${$field} !== null && (!\is_int(${$field}) || ${$field} < $range[0] || ${$field} > $range[1])) {
                throw new \MailPoetVendor\Carbon\Exceptions\InvalidDateException($field, ${$field});
            }
        }
    ...
    protected static function translator()
    {
        if (static::$translator === null) {
            static::$translator = \MailPoetVendor\Carbon\Translator::get();
        }
     ...
     * @return void
     */
    public static function setTranslator(\MailPoetVendor\Symfony\Component\Translation\TranslatorInterface $translator)
    {
    ...
    {
        return static::executeWithLocale($locale, function ($newLocale, \MailPoetVendor\Symfony\Component\Translation\TranslatorInterface $translator) {
    ...
    public static function getAvailableLocales()
    {
        ...
        $locales = array();
        if ($translator instanceof \MailPoetVendor\Carbon\Translator) {
    ...
    {
        return $this->copy()->setTimezone('UTC')->format('MailPoetVendor\\Y-m-d\\TH:i:s\\Z');
    }
    /**
    ...
    {
        return $this->format('MailPoetVendor\\D M j Y H:i:s \\G\\M\\TO');
    }
    /**
     ...
     * @return bool
     */
    public function between($date1, $date2, $equal = \true)
    {
        if ($date1->gt($date2)) {
            $temp = $date1;
            $date1 = $date2;
     ...
     * @return bool
     */
    public function isBetween($date1, $date2, $equal = \true)
    {
    ...
    {
        $diff = \MailPoetVendor\Carbon\CarbonInterval::instance($diff, $trimMicroseconds);
        // @codeCoverageIgnoreStart
     ...
     * @return CarbonInterval
     */
    public function diffAsCarbonInterval($date = null, $absolute = \true, $trimMicroseconds = \true)
    {
        $from = $this;
    ...
    {
        return $this->diffFiltered(\MailPoetVendor\Carbon\CarbonInterval::day(), $callback, $date, $absolute);
    }
    /**
    ...
    {
        return $this->diffFiltered(\MailPoetVendor\Carbon\CarbonInterval::hour(), $callback, $date, $absolute);
    }
    /**
     ...
     * @return int
     */
    public function diffFiltered(\MailPoetVendor\Carbon\CarbonInterval $ci, \Closure $callback, $date = null, $absolute = \true)
    {
        $start = $this;
        ...
        $values = \array_filter(\iterator_to_array($period), function ($date) use($callback) {
            return \call_user_func($callback, \MailPoetVendor\Carbon\Carbon::instance($date));
        });
        $diff = \count($values);
    ...
    {
        return $this->diffInDaysFiltered(function (\MailPoetVendor\Carbon\Carbon $date) {
    ...
    public function diffInMicroseconds($date = null, $absolute = \true)
    {
        ...
        $value = (int) \round(((($diff->days * static::HOURS_PER_DAY + $diff->h) * static::MINUTES_PER_HOUR + $diff->i) * static::SECONDS_PER_MINUTE + ($micro + $diff->s)) * static::MICROSECONDS_PER_SECOND);
        return $absolute || !$diff->invert ? $value : -$value;
    }
    /**

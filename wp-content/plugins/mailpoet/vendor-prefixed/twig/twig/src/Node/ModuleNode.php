 ...
 */
class ModuleNode extends \MailPoetVendor\Twig\Node\Node
{
    public function __construct(\MailPoetVendor\Twig_NodeInterface $body, \MailPoetVendor\Twig\Node\Expression\AbstractExpression $parent = null, \MailPoetVendor\Twig_NodeInterface $blocks, \MailPoetVendor\Twig_NodeInterface $macros, \MailPoetVendor\Twig_NodeInterface $traits, $embeddedTemplates, $name, $source = '')
    {
        if (!$name instanceof \MailPoetVendor\Twig\Source) {
        ...
        $nodes = ['body' => $body, 'blocks' => $blocks, 'macros' => $macros, 'traits' => $traits, 'display_start' => new \MailPoetVendor\Twig\Node\Node(), 'display_end' => new \MailPoetVendor\Twig\Node\Node(), 'constructor_start' => new \MailPoetVendor\Twig\Node\Node(), 'constructor_end' => new \MailPoetVendor\Twig\Node\Node(), 'class_end' => new \MailPoetVendor\Twig\Node\Node()];
        if (null !== $parent) {
            $nodes['parent'] = $parent;
        }
    ...
    protected function compileTemplate(\MailPoetVendor\Twig\Compiler $compiler)
    {
        if (!$this->getAttribute('index')) {
            $compiler->write('<?php');
        }
        $this->compileClassHeader($compiler);
        if (\count($this->getNode('blocks')) || \count($this->getNode('traits')) || !$this->hasNode('parent') || $this->getNode('parent') instanceof \MailPoetVendor\Twig\Node\Expression\ConstantExpression || \count($this->getNode('constructor_start')) || \count($this->getNode('constructor_end'))) {
    ...
    protected function compileGetParent(\MailPoetVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileClassHeader(\MailPoetVendor\Twig\Compiler $compiler)
    {
        $compiler->write("\n\n");
        if (!$this->getAttribute('index')) {
    ...
    protected function compileConstructor(\MailPoetVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileDisplay(\MailPoetVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileClassFooter(\MailPoetVendor\Twig\Compiler $compiler)
    {
        $compiler->subcompile($this->getNode('class_end'))->outdent()->write("}\n");
    }
    protected function compileMacros(\MailPoetVendor\Twig\Compiler $compiler)
    {
        $compiler->subcompile($this->getNode('macros'));
    }
    protected function compileGetTemplateName(\MailPoetVendor\Twig\Compiler $compiler)
    {
        $compiler->write("public function getTemplateName()\n", "{\n")->indent()->write('return ')->repr($this->getSourceContext()->getName())->raw(";\n")->outdent()->write("}\n\n");
    }
    protected function compileIsTraitable(\MailPoetVendor\Twig\Compiler $compiler)
    {
        ...
        if ($traitable) {
            if ($this->getNode('body') instanceof \MailPoetVendor\Twig\Node\BodyNode) {
                $nodes = $this->getNode('body')->getNode(0);
            } else {
                $nodes = $this->getNode('body');
            }
            if (!\count($nodes)) {
                $nodes = new \MailPoetVendor\Twig\Node\Node([$nodes]);
            }
            foreach ($nodes as $node) {
                ...
                if ($node instanceof \MailPoetVendor\Twig\Node\BlockReferenceNode) {
                    continue;
                }
                $traitable = \false;
                break;
    ...
    protected function compileDebugInfo(\MailPoetVendor\Twig\Compiler $compiler)
    {
        $compiler->write("public function getDebugInfo()\n", "{\n")->indent()->write(\sprintf("return %s;\n", \str_replace("\n", '', \var_export(\array_reverse($compiler->getDebugInfo(), \true), \true))))->outdent()->write("}\n\n");
    }
    protected function compileGetSource(\MailPoetVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileGetSourceContext(\MailPoetVendor\Twig\Compiler $compiler)
    {
    ...
    protected function compileLoadTemplate(\MailPoetVendor\Twig\Compiler $compiler, $node, $var)
    {

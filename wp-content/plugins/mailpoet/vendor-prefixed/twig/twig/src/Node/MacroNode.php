 ...
 */
class MacroNode extends \MailPoetVendor\Twig\Node\Node
{
    const VARARGS_NAME = 'varargs';
    public function __construct($name, \MailPoetVendor\Twig_NodeInterface $body, \MailPoetVendor\Twig_NodeInterface $arguments, $lineno, $tag = null)
    {
        foreach ($arguments as $argumentName => $argument) {
            if (self::VARARGS_NAME === $argumentName) {
                throw new \MailPoetVendor\Twig\Error\SyntaxError(\sprintf('The argument "%s" in macro "%s" cannot be defined because the variable "%s" is reserved for arbitrary arguments.', self::VARARGS_NAME, $name, self::VARARGS_NAME), $argument->getTemplateLine(), $argument->getSourceContext());
            }
        }

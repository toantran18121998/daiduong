 ...
 * @author Chris Corbyn
 */
class Swift_StreamFilters_StringReplacementFilterFactory implements \MailPoetVendor\Swift_ReplacementFilterFactory
{
    /** Lazy-loaded filters */
    private $filters = [];
    /**
    ...
    public function createFilter($search, $replace)
    {
        if (!isset($this->filters[$search][$replace])) {
            ...
            $this->filters[$search][$replace] = new \MailPoetVendor\Swift_StreamFilters_StringReplacementFilter($search, $replace);
        }

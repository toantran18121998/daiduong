 ...
 */
class Swift_Transport_Esmtp_Auth_NTLMAuthenticator implements \MailPoetVendor\Swift_Transport_Esmtp_Authenticator
{
     ...
     */
    public function authenticate(\MailPoetVendor\Swift_Transport_SmtpAgent $agent, $username, $password)
    {
        ...
        try {
            ...
            $timestamp = \func_num_args() > 3 ? \func_get_arg(3) : $this->getCorrectTimestamp(\bcmul(\microtime(\true), '1000'));
     ...
     */
    protected function sendMessage1(\MailPoetVendor\Swift_Transport_SmtpAgent $agent)
    {
    ...
    protected function parseMessage2($response)
    {
        ...
        return [$challenge, $context, $targetInfoH, $targetName, $domainName, $serverName, $DNSDomainName, $DNSServerName, \hex2bin($targetInfoBlock), $terminatorByte];
    }
    /**
     ...
     * @return string
     */
    protected function sendMessage3($response, $username, $password, $timestamp, $client, \MailPoetVendor\Swift_Transport_SmtpAgent $agent, $v2 = \true)
    {
    ...
    protected function createMessage3($domain, $username, $workstation, $lmResponse, $ntlmResponse)
    {
        ...
        $lmSec = $this->createSecurityBuffer($lmResponse, ($workInfo[0] + $workInfo[1]) / 2, \true);
        $lmInfo = $this->readSecurityBuffer(\bin2hex($lmSec));
        $ntlmSec = $this->createSecurityBuffer($ntlmResponse, ($lmInfo[0] + $lmInfo[1]) / 2, \true);
    ...
    protected function createDesKey($key)
    {
        ...
        foreach ($material as $k => $v) {
            ...
            $needsParity = 0 == (($this->uRShift($b, 7) ^ $this->uRShift($b, 6) ^ $this->uRShift($b, 5) ^ $this->uRShift($b, 4) ^ $this->uRShift($b, 3) ^ $this->uRShift($b, 2) ^ $this->uRShift($b, 1)) & 0x1);
    ...
    protected function debug($message)
    {
        ...
        if ('02000000' == $messageId) {
            $map = ['Challenge', 'Context', 'Target Information Security Buffer', 'Target Name Data', 'NetBIOS Domain Name', 'NetBIOS Server Name', 'DNS Domain Name', 'DNS Server Name', 'BLOB', 'Target Information Terminator'];
            ...
            $map = ['LM Response Security Buffer', 'NTLM Response Security Buffer', 'Target Name Security Buffer', 'User Name Security Buffer', 'Workstation Name Security Buffer', 'Session Key Security Buffer', 'Flags', 'Target Name Data', 'User Name Data', 'Workstation Name Data', 'LM Response Data', 'NTLM Response Data'];
            foreach ($map as $key => $value) {

 ...
 * @author Chris Corbyn
 */
class Swift_Mime_Headers_MailboxHeader extends \MailPoetVendor\Swift_Mime_Headers_AbstractHeader
{
    /**
     ...
     */
    public function __construct($name, \MailPoetVendor\Swift_Mime_HeaderEncoder $encoder, \MailPoetVendor\Egulias\EmailValidator\EmailValidator $emailValidator, \MailPoetVendor\Swift_AddressEncoder $addressEncoder = null)
    {
    ...
    {
        if (!$this->emailValidator->isValid($address, new \MailPoetVendor\Egulias\EmailValidator\Validation\RFCValidation())) {
            throw new \MailPoetVendor\Swift_RfcComplianceException('Address in mailbox given [' . $address . '] does not comply with RFC 2822, 3.6.2.');
        }
    }
}

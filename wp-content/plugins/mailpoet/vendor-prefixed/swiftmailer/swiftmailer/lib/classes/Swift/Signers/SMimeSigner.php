 ...
 */
class Swift_Signers_SMimeSigner implements \MailPoetVendor\Swift_Signers_BodySigner
{
    ...
    public function __construct($signCertificate = null, $signPrivateKey = null, $encryptCertificate = null)
    {
        ...
        $this->replacementFactory = \MailPoetVendor\Swift_DependencyContainer::getInstance()->lookup('transport.replacementfactory');
     ...
     * @return $this
     */
    public function signMessage(\MailPoetVendor\Swift_Message $message)
    {
     ...
     */
    protected function smimeSignMessage(\MailPoetVendor\Swift_Message $message)
    {
        ...
        $messageStream = new \MailPoetVendor\Swift_ByteStream_TemporaryFileByteStream();
        $signMessage->toByteStream($messageStream);
        $messageStream->commit();
        $signedMessageStream = new \MailPoetVendor\Swift_ByteStream_TemporaryFileByteStream();
        ...
        if (!\openssl_pkcs7_sign($messageStream->getPath(), $signedMessageStream->getPath(), $this->signCertificate, $this->signPrivateKey, [], $this->signOptions, $this->extraCerts)) {
            throw new \MailPoetVendor\Swift_IoException(\sprintf('Failed to sign S/Mime message. Error: "%s".', \openssl_error_string()));
        }
     ...
     */
    protected function smimeEncryptMessage(\MailPoetVendor\Swift_Message $message)
    {
        ...
        $messageStream->commit();
        $encryptedMessageStream = new \MailPoetVendor\Swift_ByteStream_TemporaryFileByteStream();
        // Encrypt the message
        if (!\openssl_pkcs7_encrypt($messageStream->getPath(), $encryptedMessageStream->getPath(), $this->encryptCert, [], 0, $this->encryptCipher)) {
            throw new \MailPoetVendor\Swift_IoException(\sprintf('Failed to encrypt S/Mime message. Error: "%s".', \openssl_error_string()));
        }
        // Parse the resulting signed message content back into the Swift message
        // preserving the original headers
     ...
     */
    protected function copyHeaders(\MailPoetVendor\Swift_Message $fromMessage, \MailPoetVendor\Swift_Message $toMessage, array $headers = [])
    {
     ...
     */
    protected function copyHeader(\MailPoetVendor\Swift_Message $fromMessage, \MailPoetVendor\Swift_Message $toMessage, $headerName)
    {
        ...
        switch ($header->getFieldType()) {
            case \MailPoetVendor\Swift_Mime_Header::TYPE_TEXT:
                $headers->addTextHeader($header->getFieldName(), $header->getValue());
                break;
            case \MailPoetVendor\Swift_Mime_Header::TYPE_PARAMETERIZED:
     ...
     */
    protected function clearAllHeaders(\MailPoetVendor\Swift_Message $message)
    {
     ...
     */
    protected function wrapMimeMessage(\MailPoetVendor\Swift_Message $message)
    {
        ...
        $wrappedMessage = new \MailPoetVendor\Swift_MimePart($messageStream, 'message/rfc822');
        $wrappedMessage->setEncoder(new \MailPoetVendor\Swift_Mime_ContentEncoder_PlainContentEncoder('7bit'));
        return $wrappedMessage;
    }
    protected function parseSSLOutput(\MailPoetVendor\Swift_InputByteStream $inputStream, \MailPoetVendor\Swift_Message $message)
    {
        $messageStream = new \MailPoetVendor\Swift_ByteStream_TemporaryFileByteStream();
     ...
     */
    protected function streamToMime(\MailPoetVendor\Swift_OutputByteStream $fromStream, \MailPoetVendor\Swift_Message $message)
    {
        ...
        $message->setEncoder(new \MailPoetVendor\Swift_Mime_ContentEncoder_NullContentEncoder($encoding));
     ...
     * @return array
     */
    protected function parseStream(\MailPoetVendor\Swift_OutputByteStream $emailStream)
    {
        $bufferLength = 78;
        $headerData = '';
        $headerBodySeparator = "\r\n\r\n";
        ...
        $bodyStream = new \MailPoetVendor\Swift_ByteStream_TemporaryFileByteStream();
    ...
    protected function copyFromOpenSSLOutput(\MailPoetVendor\Swift_OutputByteStream $fromStream, \MailPoetVendor\Swift_InputByteStream $toStream)
    {
        $bufferLength = 4096;
        $filteredStream = new \MailPoetVendor\Swift_ByteStream_TemporaryFileByteStream();

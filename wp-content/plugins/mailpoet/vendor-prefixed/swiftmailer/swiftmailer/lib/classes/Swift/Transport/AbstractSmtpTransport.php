 ...
 * @author Chris Corbyn
 */
abstract class Swift_Transport_AbstractSmtpTransport implements \MailPoetVendor\Swift_Transport
{
     ...
     */
    public function __construct(\MailPoetVendor\Swift_Transport_IoBuffer $buf, \MailPoetVendor\Swift_Events_EventDispatcher $dispatcher, $localDomain = '127.0.0.1', \MailPoetVendor\Swift_AddressEncoder $addressEncoder = null)
    {
        $this->buffer = $buf;
        $this->eventDispatcher = $dispatcher;
        $this->addressEncoder = $addressEncoder ?? new \MailPoetVendor\Swift_AddressEncoder_IdnAddressEncoder();
    ...
    public function setAddressEncoder(\MailPoetVendor\Swift_AddressEncoder $addressEncoder)
    {
    ...
    public function start()
    {
        if (!$this->started) {
            ...
            } catch (\MailPoetVendor\Swift_TransportException $e) {
    ...
    public function send(\MailPoetVendor\Swift_Mime_SimpleMessage $message, &$failedRecipients = null)
    {
        ...
        if (!($reversePath = $this->getReversePath($message))) {
            $this->throwException(new \MailPoetVendor\Swift_TransportException('Cannot send message without a sender address'));
        }
        ...
        if ($evt) {
            if ($sent == \count($to) + \count($cc) + \count($bcc)) {
                $evt->setResult(\MailPoetVendor\Swift_Events_SendEvent::RESULT_SUCCESS);
            } elseif ($sent > 0) {
                $evt->setResult(\MailPoetVendor\Swift_Events_SendEvent::RESULT_TENTATIVE);
            } else {
                $evt->setResult(\MailPoetVendor\Swift_Events_SendEvent::RESULT_FAILED);
            }
    ...
    protected function streamMessage(\MailPoetVendor\Swift_Mime_SimpleMessage $message)
    {
    ...
    protected function getReversePath(\MailPoetVendor\Swift_Mime_SimpleMessage $message)
    {
    ...
    protected function throwException(\MailPoetVendor\Swift_TransportException $e)
    {
    ...
    {
        if (!$response) {
            $this->throwException(new \MailPoetVendor\Swift_TransportException('Expected response code ' . \implode('/', $wanted) . ' but got an empty response'));
        }
        ...
        if (!$valid) {
            $this->throwException(new \MailPoetVendor\Swift_TransportException('Expected response code ' . \implode('/', $wanted) . ' but got code "' . $code . '", with message "' . $response . '"', $code));
        }
    }
    ...
    protected function getFullResponse($seq)
    {
        ...
        try {
            ...
            $this->throwException($e);
        } catch (\MailPoetVendor\Swift_IoException $e) {
            $this->throwException(new \MailPoetVendor\Swift_TransportException($e->getMessage(), 0, $e));
        }
        return $response;
    }
    ...
    private function doMailTransaction($message, $reversePath, array $recipients, array &$failedRecipients)
    {
        ...
        foreach ($recipients as $forwardPath) {
            ...
            } catch (\MailPoetVendor\Swift_AddressEncoderException $e) {
                $failedRecipients[] = $forwardPath;
            }
        }
        if (0 != $sent) {
    ...
    private function sendTo(\MailPoetVendor\Swift_Mime_SimpleMessage $message, $reversePath, array $to, array &$failedRecipients)
    {
        if (empty($to)) {
            return 0;
        }
    ...
    private function sendBcc(\MailPoetVendor\Swift_Mime_SimpleMessage $message, $reversePath, array $bcc, array &$failedRecipients)
    {
        $sent = 0;
        foreach ($bcc as $forwardPath => $name) {
            $message->setBcc([$forwardPath => $name]);
            $sent += $this->doMailTransaction($message, $reversePath, [$forwardPath], $failedRecipients);
        }
        return $sent;
    }
    /**

 ...
 * @author     Chris Corbyn
 */
class Swift_Transport_StreamBuffer extends \MailPoetVendor\Swift_ByteStream_AbstractFilterableInputStream implements \MailPoetVendor\Swift_Transport_IoBuffer
{
    /** A primary socket */
    private $stream;
    /** The input stream */
     ...
     */
    public function __construct(\MailPoetVendor\Swift_ReplacementFilterFactory $replacementFactory)
    {
    ...
    public function startTLS()
    {
        ...
        return \stream_socket_enable_crypto($this->stream, \true, \STREAM_CRYPTO_METHOD_TLSv1_0_CLIENT | \STREAM_CRYPTO_METHOD_TLSv1_1_CLIENT | \STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT);
    }
    /**
    ...
    public function readLine($sequence)
    {
        if (isset($this->out) && !\feof($this->out)) {
            ...
            if (0 == \strlen($line)) {
                ...
                if ($metas['timed_out']) {
                    throw new \MailPoetVendor\Swift_IoException('Connection to ' . $this->getReadConnectionDescription() . ' Timed Out');
                }
            }
            return $line;
        }
    ...
    private function establishSocketConnection()
    {
        ...
        \set_error_handler(function ($type, $msg) {
            throw new \MailPoetVendor\Swift_TransportException('Connection could not be established with host ' . $this->params['host'] . ' :' . $msg);
        });
        try {
    ...
    private function establishProcessConnection()
    {
        ...
        if ($err = \stream_get_contents($pipes[2])) {
            throw new \MailPoetVendor\Swift_TransportException('Process could not be started [' . $err . ']');
        }
        $this->in =& $pipes[0];
        $this->out =& $pipes[1];
    }

 ...
 * @author Chris Corbyn
 */
class Swift_KeyCache_DiskKeyCache implements \MailPoetVendor\Swift_KeyCache
{
     ...
     */
    public function __construct(\MailPoetVendor\Swift_KeyCache_KeyCacheInputStream $stream, $path)
    {
        $this->stream = $stream;
        $this->path = $path;
    }
    ...
    public function setString($nsKey, $itemKey, $string, $mode)
    {
        ...
        switch ($mode) {
            ...
            case self::MODE_APPEND:
                ...
                break;
            default:
                throw new \MailPoetVendor\Swift_SwiftException('Invalid mode [' . $mode . '] used to set nsKey=' . $nsKey . ', itemKey=' . $itemKey);
                break;
        }
        \fwrite($fp, $string);
     ...
     * @throws Swift_IoException
     */
    public function importFromByteStream($nsKey, $itemKey, \MailPoetVendor\Swift_OutputByteStream $os, $mode)
    {
        $this->prepareCache($nsKey);
        switch ($mode) {
            case self::MODE_WRITE:
     ...
     */
    public function getInputByteStream($nsKey, $itemKey, \MailPoetVendor\Swift_InputByteStream $writeThrough = null)
    {
     ...
     */
    public function exportToByteStream($nsKey, $itemKey, \MailPoetVendor\Swift_InputByteStream $is)
    {
        if ($this->hasKey($nsKey, $itemKey)) {
            $fp = $this->getHandle($nsKey, $itemKey, self::POSITION_START);
            while (!\feof($fp) && \false !== ($bytes = \fread($fp, 8192))) {
    ...
    private function prepareCache($nsKey)
    {
        ...
        if (!\is_dir($cacheDir)) {
            if (!\mkdir($cacheDir)) {
                throw new \MailPoetVendor\Swift_IoException('Failed to create cache directory ' . $cacheDir);
            }
            $this->keys[$nsKey] = [];
        }
    }

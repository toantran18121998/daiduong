 ...
 * @author Chris Corbyn
 */
class Swift_Mime_SimpleMimeEntity implements \MailPoetVendor\Swift_Mime_CharsetObserver, \MailPoetVendor\Swift_Mime_EncodingObserver
{
     ...
     */
    public function __construct(\MailPoetVendor\Swift_Mime_SimpleHeaderSet $headers, \MailPoetVendor\Swift_Mime_ContentEncoder $encoder, \MailPoetVendor\Swift_KeyCache $cache, \MailPoetVendor\Swift_IdGenerator $idGenerator)
    {
    ...
    {
        return $this->body instanceof \MailPoetVendor\Swift_OutputByteStream ? $this->readStream($this->body) : $this->body;
    }
    /**
     ...
     * @return $this
     */
    public function setEncoder(\MailPoetVendor\Swift_Mime_ContentEncoder $encoder)
    {
     ...
     * has changed.
     */
    public function encoderChanged(\MailPoetVendor\Swift_Mime_ContentEncoder $encoder)
    {
        $this->notifyEncoderChanged($encoder);
    }
    /**
    ...
    protected function bodyToString()
    {
        ...
        if (isset($this->body) && empty($this->immediateChildren)) {
            if ($this->cache->hasKey($this->cacheKey, 'body')) {
                ...
                $this->cache->setString($this->cacheKey, 'body', $body, \MailPoetVendor\Swift_KeyCache::MODE_WRITE);
            }
            $string .= $body;
        }
     ...
     */
    public function toByteStream(\MailPoetVendor\Swift_InputByteStream $is)
    {
     ...
     */
    protected function bodyToByteStream(\MailPoetVendor\Swift_InputByteStream $is)
    {
        if (empty($this->immediateChildren)) {
            if (isset($this->body)) {
                if ($this->cache->hasKey($this->cacheKey, 'body')) {
                    ...
                    $is->write("\r\n");
                    if ($this->body instanceof \MailPoetVendor\Swift_OutputByteStream) {
    ...
    private function readStream(\MailPoetVendor\Swift_OutputByteStream $os)
    {
        $string = '';
    ...
    private function assertValidBoundary($boundary)
    {
        if (!\preg_match('/^[a-z0-9\'\\(\\)\\+_\\-,\\.\\/:=\\?\\ ]{0,69}[a-z0-9\'\\(\\)\\+_\\-,\\.\\/:=\\?]$/Di', $boundary)) {
            throw new \MailPoetVendor\Swift_RfcComplianceException('Mime boundary set is not RFC 2046 compliant.');
        }
    }
    ...
    private function notifyEncoderChanged(\MailPoetVendor\Swift_Mime_ContentEncoder $encoder)
    {
        foreach ($this->immediateChildren as $child) {
    ...
    {
        if ($this->cache instanceof \MailPoetVendor\Swift_KeyCache) {

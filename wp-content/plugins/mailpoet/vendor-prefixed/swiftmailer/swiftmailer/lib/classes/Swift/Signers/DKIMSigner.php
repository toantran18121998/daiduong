 ...
 */
class Swift_Signers_DKIMSigner implements \MailPoetVendor\Swift_Signers_HeaderSigner
{
    /**
     * PrivateKey.
     *
     ...
     * @var array
     */
    protected $ignoredHeaders = ['return-path' => \true];
    /**
     * Signer identity.
     *
     * @var string
     ...
     */
    public function bind(\MailPoetVendor\Swift_InputByteStream $is)
    {
     ...
     */
    public function unbind(\MailPoetVendor\Swift_InputByteStream $is)
    {
        // Don't have to mirror anything
        ...
        switch ($hash) {
            ...
            case 'rsa-sha256':
                ...
                if (!\defined('OPENSSL_ALGO_SHA256')) {
                    throw new \MailPoetVendor\Swift_SwiftException('Unable to set sha256 as it is not supported by OpenSSL.');
                }
                break;
            default:
                throw new \MailPoetVendor\Swift_SwiftException('Unable to set the hash algorithm, must be one of rsa-sha1 or rsa-sha256 (%s given).', $hash);
        }
        return $this;
    }
    /**
    ...
    {
        $this->ignoredHeaders[\strtolower($header_name)] = \true;
        return $this;
    }
    /**
    ...
    public function addSignature(\MailPoetVendor\Swift_Mime_SimpleHeaderSet $headers)
    {
        ...
        if (\true === $this->signatureTimestamp) {
            $params['t'] = \time();
    ...
    protected function endOfBody()
    {
        ...
        $this->bodyHash = \hash_final($this->bodyHashHandler, \true);
    }
    ...
    private function getEncryptedHash()
    {
        ...
        if (!$pkeyId) {
            throw new \MailPoetVendor\Swift_SwiftException('Unable to load DKIM Private Key [' . \openssl_error_string() . ']');
        }
        if (\openssl_sign($this->headerCanonData, $signature, $pkeyId, $algorithm)) {
            return $signature;
        }
        throw new \MailPoetVendor\Swift_SwiftException('Unable to sign DKIM Hash [' . \openssl_error_string() . ']');
    }
}

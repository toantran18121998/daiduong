 ...
 */
class Swift_FileSpool extends \MailPoetVendor\Swift_ConfigurableSpool
{
    /** The spool directory */
    private $path;
    /**
    ...
    public function __construct($path)
    {
        ...
        if (!\file_exists($this->path)) {
            if (!\mkdir($this->path, 0777, \true)) {
                throw new \MailPoetVendor\Swift_IoException(\sprintf('Unable to create path "%s".', $this->path));
            }
        }
    }
    /**
     ...
     * @return bool
     */
    public function queueMessage(\MailPoetVendor\Swift_Mime_SimpleMessage $message)
    {
        ...
        throw new \MailPoetVendor\Swift_IoException(\sprintf('Unable to create a file for enqueuing Message in "%s".', $this->path));
    }
    /**
     ...
     */
    public function flushQueue(\MailPoetVendor\Swift_Transport $transport, &$failedRecipients = null)
    {

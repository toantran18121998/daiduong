 ...
 * @author Chris Corbyn
 */
class Swift_Message extends \MailPoetVendor\Swift_Mime_SimpleMessage
{
    /**
    ...
    {
        \call_user_func_array([$this, 'MailPoetVendor\\Swift_Mime_SimpleMessage::__construct'], \MailPoetVendor\Swift_DependencyContainer::getInstance()->createDependenciesFor('mime.message'));
        if (!isset($charset)) {
            $charset = \MailPoetVendor\Swift_DependencyContainer::getInstance()->lookup('properties.charset');
        }
    ...
    {
        return $this->attach((new \MailPoetVendor\Swift_MimePart($body, $contentType, $charset))->setEncoder($this->getEncoder()));
    }
    /**
     ...
     * @return $this
     */
    public function attachSigner(\MailPoetVendor\Swift_Signer $signer)
    {
        if ($signer instanceof \MailPoetVendor\Swift_Signers_HeaderSigner) {
            $this->headerSigners[] = $signer;
        } elseif ($signer instanceof \MailPoetVendor\Swift_Signers_BodySigner) {
     ...
     * @return $this
     */
    public function detachSigner(\MailPoetVendor\Swift_Signer $signer)
    {
        if ($signer instanceof \MailPoetVendor\Swift_Signers_HeaderSigner) {
    ...
    {
        \MailPoetVendor\Swift_DependencyContainer::getInstance()->createDependenciesFor('mime.message');
    }
    /**

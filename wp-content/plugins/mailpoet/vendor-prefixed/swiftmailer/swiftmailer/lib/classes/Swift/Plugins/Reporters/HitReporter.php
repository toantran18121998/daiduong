 ...
 * @author Chris Corbyn
 */
class Swift_Plugins_Reporters_HitReporter implements \MailPoetVendor\Swift_Plugins_Reporter
{
    /**
     ...
     */
    public function notify(\MailPoetVendor\Swift_Mime_SimpleMessage $message, $address, $result)
    {
        if (self::RESULT_FAIL == $result && !isset($this->failures_cache[$address])) {
            $this->failures[] = $address;
            $this->failures_cache[$address] = \true;
        }
    }
    /**

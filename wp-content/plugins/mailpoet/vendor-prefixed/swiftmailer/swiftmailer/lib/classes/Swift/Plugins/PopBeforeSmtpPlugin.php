 ...
 * @author     Chris Corbyn
 */
class Swift_Plugins_PopBeforeSmtpPlugin implements \MailPoetVendor\Swift_Events_TransportChangeListener, \MailPoetVendor\Swift_Plugins_Pop_Pop3Connection
{
     ...
     * @return $this
     */
    public function setConnection(\MailPoetVendor\Swift_Plugins_Pop_Pop3Connection $connection)
    {
     ...
     */
    public function bindSmtp(\MailPoetVendor\Swift_Transport $smtp)
    {
        $this->transport = $smtp;
    }
    /**
        ...
        if (isset($this->connection)) {
            ...
            if (!isset($this->socket)) {
                if (!($socket = \fsockopen($this->getHostString(), $this->port, $errno, $errstr, $this->timeout))) {
                    throw new \MailPoetVendor\Swift_Plugins_Pop_Pop3Exception(\sprintf('Failed to connect to POP3 host [%s]: %s', $this->host, $errstr));
                }
                $this->socket = $socket;
                if (\false === ($greeting = \fgets($this->socket))) {
                    throw new \MailPoetVendor\Swift_Plugins_Pop_Pop3Exception(\sprintf('Failed to connect to POP3 host [%s]', \trim($greeting)));
                }
    ...
    public function disconnect()
    {
        if (isset($this->connection)) {
            ...
            if (!\fclose($this->socket)) {
                throw new \MailPoetVendor\Swift_Plugins_Pop_Pop3Exception(\sprintf('POP3 host [%s] connection could not be stopped', $this->host));
            }
            $this->socket = null;
        }
    }
    ...
    private function command($command)
    {
        if (!\fwrite($this->socket, $command)) {
            throw new \MailPoetVendor\Swift_Plugins_Pop_Pop3Exception(\sprintf('Failed to write command [%s] to POP3 host', \trim($command)));
        }
        if (\false === ($response = \fgets($this->socket))) {
            throw new \MailPoetVendor\Swift_Plugins_Pop_Pop3Exception(\sprintf('Failed to read from POP3 host after command [%s]', \trim($command)));
        }
    ...
    private function assertOk($response)
    {
        if ('+OK' != \substr($response, 0, 3)) {
            throw new \MailPoetVendor\Swift_Plugins_Pop_Pop3Exception(\sprintf('POP3 command failed [%s]', \trim($response)));
        }
    }

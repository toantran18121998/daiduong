...
class OutputFormat
{
    ...
    public function __call($sMethodName, $aArguments)
    {
        if (\strpos($sMethodName, 'set') === 0) {
            ...
            if (\strpos($sMethodName, 'get') === 0) {
                return $this->get(\substr($sMethodName, 3));
            } else {
                if (\method_exists('MailPoetVendor\\Sabberworm\\CSS\\OutputFormatter', $sMethodName)) {
    ...
    public function getFormatter()
    {
        if ($this->oFormatter === null) {
            $this->oFormatter = new \MailPoetVendor\Sabberworm\CSS\OutputFormatter($this);
        }
    ...
    {
        return new \MailPoetVendor\Sabberworm\CSS\OutputFormat();
    }
    /**
...
{
    private $oFormat;
    public function __construct(\MailPoetVendor\Sabberworm\CSS\OutputFormat $oFormat)
    {
    ...
    public function safely($cCode)
    {
        if ($this->oFormat->get('IgnoreExceptions')) {
            ...
            try {
                return $cCode();
            } catch (\MailPoetVendor\Sabberworm\CSS\Parsing\OutputException $e) {
                return null;
            }
            //Do nothing
        } else {
    ...
    public function implode($sSeparator, $aValues, $bIncreaseLevel = \false)
    {
        ...
        foreach ($aValues as $mValue) {
            ...
            if ($mValue instanceof \MailPoetVendor\Sabberworm\CSS\Renderable) {

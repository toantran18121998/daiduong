 ...
 */
abstract class RuleSet implements \MailPoetVendor\Sabberworm\CSS\Renderable, \MailPoetVendor\Sabberworm\CSS\Comment\Commentable
{
    private $aRules;
    protected $iLineNo;
    protected $aComments;
    ...
    public static function parseRuleSet(\MailPoetVendor\Sabberworm\CSS\Parsing\ParserState $oParserState, \MailPoetVendor\Sabberworm\CSS\RuleSet\RuleSet $oRuleSet)
    {
        ...
        while (!$oParserState->comes('}')) {
            ...
            if ($oParserState->getSettings()->bLenientParsing) {
                try {
                    $oRule = \MailPoetVendor\Sabberworm\CSS\Rule\Rule::parse($oParserState);
    ...
    public function addRule(\MailPoetVendor\Sabberworm\CSS\Rule\Rule $oRule, \MailPoetVendor\Sabberworm\CSS\Rule\Rule $oSibling = null)
    {
        $sRule = $oRule->getRule();
    ...
    {
        if ($mRule instanceof \MailPoetVendor\Sabberworm\CSS\Rule\Rule) {
            $mRule = $mRule->getRule();
        }
        $aResult = array();

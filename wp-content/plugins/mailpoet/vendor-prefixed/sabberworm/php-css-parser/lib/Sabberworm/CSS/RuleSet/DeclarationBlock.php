 ...
 */
class DeclarationBlock extends \MailPoetVendor\Sabberworm\CSS\RuleSet\RuleSet
{
    private $aSelectors;
    ...
    {
        $aComments = array();
        $oResult = new \MailPoetVendor\Sabberworm\CSS\RuleSet\DeclarationBlock($oParserState->currentLine());
        $oResult->setSelector($oParserState->consumeUntil('{', \false, \true, $aComments));
        $oResult->setComments($aComments);
        \MailPoetVendor\Sabberworm\CSS\RuleSet\RuleSet::parseRuleSet($oParserState, $oResult);
        return $oResult;
    }
    public function setSelectors($mSelector)
    {
        ...
        foreach ($this->aSelectors as $iKey => $mSelector) {
            if (!$mSelector instanceof \MailPoetVendor\Sabberworm\CSS\Property\Selector) {
                $this->aSelectors[$iKey] = new \MailPoetVendor\Sabberworm\CSS\Property\Selector($mSelector);
            }
        }
    }
    ...
    public function expandBorderShorthand()
    {
        ...
        foreach ($aBorderRules as $sBorderRule) {
            ...
            $aValues = array();
            if (!$mRuleValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList) {
                $aValues[] = $mRuleValue;
            } else {
                $aValues = $mRuleValue->getListComponents();
            }
            foreach ($aValues as $mValue) {
                if ($mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\Value) {
                    $mNewValue = clone $mValue;
                } else {
                    $mNewValue = $mValue;
                }
                if ($mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\Size) {
                    $sNewRuleName = $sBorderRule . "-width";
                } else {
                    if ($mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\Color) {
                ...
                $oNewRule = new \MailPoetVendor\Sabberworm\CSS\Rule\Rule($sNewRuleName, $this->iLineNo);
    ...
    public function expandDimensionsShorthand()
    {
        ...
        foreach ($aExpansions as $sProperty => $sExpanded) {
            ...
            foreach (array('top', 'right', 'bottom', 'left') as $sPosition) {
                $oNewRule = new \MailPoetVendor\Sabberworm\CSS\Rule\Rule(\sprintf($sExpanded, $sPosition), $this->iLineNo);
                $oNewRule->setIsImportant($oRule->getIsImportant());
    ...
    public function expandFontShorthand()
    {
            ...
            if (\in_array($mValue, array('normal', 'inherit'))) {
                ...
                if (\in_array($mValue, array('italic', 'oblique'))) {
                    ...
                    if ($mValue == 'small-caps') {
                        ...
                        if (\in_array($mValue, array('bold', 'bolder', 'lighter')) || $mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\Size && \in_array($mValue->getSize(), \range(100, 900, 100))) {
                            $aFontProperties['font-weight'] = $mValue;
                        } else {
                            if ($mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList && $mValue->getListSeparator() == '/') {
                                list($oSize, $oHeight) = $mValue->getListComponents();
                                $aFontProperties['font-size'] = $oSize;
                                $aFontProperties['line-height'] = $oHeight;
                            } else {
                                if ($mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\Size && $mValue->getUnit() !== null) {
        ...
        foreach ($aFontProperties as $sProperty => $mValue) {
            $oNewRule = new \MailPoetVendor\Sabberworm\CSS\Rule\Rule($sProperty, $this->iLineNo);
    ...
    public function expandBackgroundShorthand()
    {
        ...
        foreach ($aValues as $mValue) {
            ...
            if ($mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\URL) {
                ...
                if ($mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\Color) {
                    ...
                    if (\in_array($mValue, array('scroll', 'fixed'))) {
                        ...
                        if (\in_array($mValue, array('repeat', 'no-repeat', 'repeat-x', 'repeat-y'))) {
                            ...
                            if (\in_array($mValue, array('left', 'center', 'right', 'top', 'bottom')) || $mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\Size) {
                                if ($iNumBgPos == 0) {
    ...
    public function createShorthandProperties(array $aProperties, $sShorthand)
    {
        ...
        if (\count($aNewValues)) {
            $oNewRule = new \MailPoetVendor\Sabberworm\CSS\Rule\Rule($sShorthand, $this->iLineNo);
    ...
    public function createFontShorthand()
    {
        ...
        $oNewRule = new \MailPoetVendor\Sabberworm\CSS\Rule\Rule('font', $this->iLineNo);
        ...
        if (isset($aRules['line-height'])) {
            ...
            if ($aLHValues[0] !== 'normal') {
                $val = new \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList('/', $this->iLineNo);
        ...
        $oFFValue = new \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList(',', $this->iLineNo);
    ...
    public function render(\MailPoetVendor\Sabberworm\CSS\OutputFormat $oOutputFormat)
    {
        if (\count($this->aSelectors) === 0) {
            ...
            throw new \MailPoetVendor\Sabberworm\CSS\Parsing\OutputException("Attempt to print declaration block with missing selector", $this->iLineNo);
        }

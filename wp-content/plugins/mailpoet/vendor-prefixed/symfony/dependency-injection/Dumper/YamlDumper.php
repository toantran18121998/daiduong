...
use MailPoetVendor\Symfony\Component\Yaml\Dumper as YmlDumper;
use MailPoetVendor\Symfony\Component\Yaml\Parser;
 ...
 */
class YamlDumper extends \MailPoetVendor\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    private $dumper;
    /**
    ...
    public function dump(array $options = [])
    {
        ...
        if (null === $this->dumper) {
            $this->dumper = new \MailPoetVendor\Symfony\Component\Yaml\Dumper();
        }
     ...
     * @return string
     */
    private function addService($id, \MailPoetVendor\Symfony\Component\DependencyInjection\Definition $definition)
    {
        $code = "    {$id}:\n";
     ...
     * @return string
     */
    private function addServiceAlias($alias, \MailPoetVendor\Symfony\Component\DependencyInjection\Alias $id)
    {
        if ($id->isPrivate()) {
            return \sprintf("    %s: '@%s'\n", $alias, $id);
        }
    ...
    private function dumpValue($value)
    {
        ...
        if ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\ArgumentInterface) {
            if ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument) {
                return new \MailPoetVendor\Symfony\Component\Yaml\Tag\TaggedValue('tagged', $value->getTag());
            }
            if ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $tag = 'iterator';
            } else {
                throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Unspecified Yaml tag for type "%s".', \get_class($value)));
            }
        ...
        if (\is_array($value)) {
            ...
            return new \MailPoetVendor\Symfony\Component\Yaml\Tag\TaggedValue('service', (new \MailPoetVendor\Symfony\Component\Yaml\Parser())->parse("_:\n" . $this->addService('_', $value), \MailPoetVendor\Symfony\Component\Yaml\Yaml::PARSE_CUSTOM_TAGS)['_']['_']);
    ...
    private function getServiceCall($id, \MailPoetVendor\Symfony\Component\DependencyInjection\Reference $reference = null)
    {
        if (null !== $reference) {
            switch ($reference->getInvalidBehavior()) {
                case \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE:
                    break;
                case \MailPoetVendor\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE:
                    return \sprintf('@!%s', $id);
                default:
                    return \sprintf('@?%s', $id);
            }
    ...
    private function prepareParameters(array $parameters, $escape = \true)
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            } elseif ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Reference || \is_string($value) && 0 === \strpos($value, '@')) {
                $value = '@' . $value;
            }
            $filtered[$key] = $value;
        }

...
class InlineServiceDefinitionsPass extends \MailPoetVendor\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass implements \MailPoetVendor\Symfony\Component\DependencyInjection\Compiler\RepeatablePassInterface
{
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        if ($value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Definition && $this->cloningIds) {
            if ($value->isShared()) {
                return $value;
            }
            $value = clone $value;
        }
        if (!$value instanceof \MailPoetVendor\Symfony\Component\DependencyInjection\Reference || !$this->container->hasDefinition($id = $this->container->normalizeId($value))) {
        ...
        if (isset($this->cloningIds[$id])) {
            ...
            $ids[] = $id;
            throw new \MailPoetVendor\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($id, \array_slice($ids, \array_search($id, $ids)));
        }
     ...
     */
    private function isInlineableDefinition($id, \MailPoetVendor\Symfony\Component\DependencyInjection\Definition $definition, \MailPoetVendor\Symfony\Component\DependencyInjection\Compiler\ServiceReferenceGraph $graph)
    {

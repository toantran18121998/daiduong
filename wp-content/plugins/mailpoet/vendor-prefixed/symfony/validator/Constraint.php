...
abstract class Constraint
{
    ...
    public static function getErrorName($errorCode)
    {
        if (!isset(static::$errorNames[$errorCode])) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\InvalidArgumentException(\sprintf('The error code "%s" does not exist for constraint of type "%s".', $errorCode, \get_called_class()));
        }
    ...
    public function __construct($options = null)
    {
        ...
        if (\is_array($options) && isset($options['value']) && !\property_exists($this, 'value')) {
            if (null === $defaultOption) {
                throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ConstraintDefinitionException(\sprintf('No default option is configured for constraint "%s".', \get_class($this)));
            }
        ...
        if (\count($invalidOptions) > 0) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\InvalidOptionsException(\sprintf('The options "%s" do not exist in constraint "%s".', \implode('", "', $invalidOptions), \get_class($this)), $invalidOptions);
        }
        if (\count($missingOptions) > 0) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\MissingOptionsException(\sprintf('The options "%s" must be set for constraint "%s".', \implode('", "', \array_keys($missingOptions)), \get_class($this)), \array_keys($missingOptions));
        }
    }
    /**
    ...
    public function __set($option, $value)
    {
        ...
        throw new \MailPoetVendor\Symfony\Component\Validator\Exception\InvalidOptionsException(\sprintf('The option "%s" does not exist in constraint "%s".', $option, \get_class($this)), [$option]);
    }
    /**

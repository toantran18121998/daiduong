 ...
 */
class IbanValidator extends \MailPoetVendor\Symfony\Component\Validator\ConstraintValidator
{
    /**
    ...
    {
        if (!$constraint instanceof \MailPoetVendor\Symfony\Component\Validator\Constraints\Iban) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\UnexpectedTypeException($constraint, \MailPoetVendor\Symfony\Component\Validator\Constraints\Iban::class);
        }
        if (null === $value || '' === $value) {
            return;
        }
        ...
        if (!\ctype_alnum($canonicalized)) {
            $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Iban::INVALID_CHARACTERS_ERROR)->addViolation();
            return;
        }
        // ...start with a two-letter country code
        $countryCode = \substr($canonicalized, 0, 2);
        if (!\ctype_alpha($countryCode)) {
            $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Iban::INVALID_COUNTRY_CODE_ERROR)->addViolation();
            return;
        }
        // ...have a format available
        if (!\array_key_exists($countryCode, self::$formats)) {
            $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Iban::NOT_SUPPORTED_COUNTRY_CODE_ERROR)->addViolation();
            return;
        }
        // ...and have a valid format
        if (!\preg_match('/^' . self::$formats[$countryCode] . '$/', $canonicalized)) {
            $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Iban::INVALID_FORMAT_ERROR)->addViolation();
            return;
        }
        ...
        if (1 !== self::bigModulo97($checkSum)) {
            $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Iban::CHECKSUM_FAILED_ERROR)->addViolation();
        }
    }

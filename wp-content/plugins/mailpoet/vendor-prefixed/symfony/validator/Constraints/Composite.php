 ...
 */
abstract class Composite extends \MailPoetVendor\Symfony\Component\Validator\Constraint
{
    /**
     * {@inheritdoc}
     *
    ...
    public function __construct($options = null)
    {
        ...
        foreach ($nestedConstraints as $constraint) {
            if (!$constraint instanceof \MailPoetVendor\Symfony\Component\Validator\Constraint) {
                ...
                throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ConstraintDefinitionException(\sprintf('The value %s is not an instance of Constraint in constraint %s', $constraint, \get_class($this)));
            }
            if ($constraint instanceof \MailPoetVendor\Symfony\Component\Validator\Constraints\Valid) {
                throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ConstraintDefinitionException(\sprintf('The constraint Valid cannot be nested inside constraint %s. You can only declare the Valid constraint directly on a field or method.', \get_class($this)));
            }
        }
        ...
        foreach ($nestedConstraints as $constraint) {
            if (\property_exists($constraint, 'groups')) {
                ...
                if (\count($excessGroups) > 0) {
                    throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ConstraintDefinitionException(\sprintf('The group(s) "%s" passed to the constraint %s should also be passed to its containing constraint %s', \implode('", "', $excessGroups), \get_class($constraint), \get_class($this)));
                }
            } else {

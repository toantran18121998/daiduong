 ...
 */
class UuidValidator extends \MailPoetVendor\Symfony\Component\Validator\ConstraintValidator
{
    ...
    {
        if (!$constraint instanceof \MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\UnexpectedTypeException($constraint, \MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid::class);
        }
        if (null === $value || '' === $value) {
            return;
        }
    ...
    private function validateLoose($value, \MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid $constraint)
    {
        // Error priority:
        ...
        for ($i = 0; $i < $l; ++$i) {
            // Check length
            if (!isset($trimmed[$i])) {
                $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid::TOO_SHORT_ERROR)->addViolation();
                return;
            }
            ...
            if ('-' === $trimmed[$i]) {
                if ($i !== $h) {
                    $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid::INVALID_HYPHEN_PLACEMENT_ERROR)->addViolation();
                    return;
                }
                $h += 5;
                continue;
            ...
            if (!\ctype_xdigit($trimmed[$i])) {
                $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid::INVALID_CHARACTERS_ERROR)->addViolation();
                return;
            }
        }
        // Check length again
        if (isset($trimmed[$i])) {
            $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid::TOO_LONG_ERROR)->addViolation();
        }
    }
    private function validateStrict($value, \MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid $constraint)
    {
        // Error priority:
        // 1. ERROR_INVALID_CHARACTERS
        // 2. ERROR_INVALID_HYPHEN_PLACEMENT
        ...
        if (!\in_array($value[self::STRICT_VERSION_POSITION], $constraint->versions)) {
            $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid::INVALID_VERSION_ERROR)->addViolation();
        }
        ...
        if (8 !== (\hexdec($value[self::STRICT_VARIANT_POSITION]) & 12)) {
            $this->context->buildViolation($constraint->message)->setParameter('{{ value }}', $this->formatValue($value))->setCode(\MailPoetVendor\Symfony\Component\Validator\Constraints\Uuid::INVALID_VARIANT_ERROR)->addViolation();
        }
    }
}

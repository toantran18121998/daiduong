 ...
 */
class GetterMetadata extends \MailPoetVendor\Symfony\Component\Validator\Mapping\MemberMetadata
{
    /**
    ...
    public function __construct($class, $property, $method = null)
    {
        if (null === $method) {
            ...
            if (\method_exists($class, $getMethod)) {
                ...
                throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ValidatorException(\sprintf('Neither of these methods exist in class %s: %s, %s, %s', $class, $getMethod, $isMethod, $hasMethod));
            }
        } elseif (!\method_exists($class, $method)) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ValidatorException(\sprintf('The %s() method does not exist in class %s.', $method, $class));
        }

 ...
 */
abstract class AbstractLoader implements \MailPoetVendor\Symfony\Component\Validator\Mapping\Loader\LoaderInterface
{
    /**
    ...
    protected function newConstraint($name, $options = null)
    {
        if (\false !== \strpos($name, '\\') && \class_exists($name)) {
            ...
            if (!isset($this->namespaces[$prefix])) {
                throw new \MailPoetVendor\Symfony\Component\Validator\Exception\MappingException(\sprintf('Undefined namespace prefix "%s"', $prefix));
            }

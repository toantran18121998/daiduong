 ...
 */
class XmlFileLoader extends \MailPoetVendor\Symfony\Component\Validator\Mapping\Loader\FileLoader
{
    /**
    ...
    protected function parseConstraints(\SimpleXMLElement $nodes)
    {
        ...
        foreach ($nodes as $node) {
            if (\count($node) > 0) {
                ...
                $options = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize(\trim($node));
            } else {
                $options = null;
            }
    ...
    protected function parseOptions(\SimpleXMLElement $nodes)
    {
        ...
        foreach ($nodes as $node) {
            if (\count($node) > 0) {
                ...
                $value = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::phpize($node);
                if (\is_string($value)) {
                    $value = \trim($value);
                }
            }
    ...
    {
        try {
            $dom = \MailPoetVendor\Symfony\Component\Config\Util\XmlUtils::loadFile($path, __DIR__ . '/schema/dic/constraint-mapping/constraint-mapping-1.0.xsd');
        } catch (\Exception $e) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\MappingException($e->getMessage(), $e->getCode(), $e);
        }
    ...
    private function loadClassMetadataFromXml(\MailPoetVendor\Symfony\Component\Validator\Mapping\ClassMetadata $metadata, \SimpleXMLElement $classDescription)
    {

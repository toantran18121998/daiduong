 ...
 */
class PropertyMetadata extends \MailPoetVendor\Symfony\Component\Validator\Mapping\MemberMetadata
{
    /**
    ...
    public function __construct($class, $name)
    {
        if (!\property_exists($class, $name)) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ValidatorException(\sprintf('Property "%s" does not exist in class "%s"', $name, $class));
        }
    ...
    protected function newReflectionMember($objectOrClassName)
    {
        ...
        while (!\property_exists($objectOrClassName, $this->getName())) {
            ...
            if (\false === $objectOrClassName) {
                throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ValidatorException(\sprintf('Property "%s" does not exist in class "%s".', $this->getName(), $originalClass));
            }
        }

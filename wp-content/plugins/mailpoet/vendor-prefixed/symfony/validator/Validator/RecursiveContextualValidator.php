 ...
 */
class RecursiveContextualValidator implements \MailPoetVendor\Symfony\Component\Validator\Validator\ContextualValidatorInterface
{
    private $context;
     ...
     */
    public function __construct(\MailPoetVendor\Symfony\Component\Validator\Context\ExecutionContextInterface $context, \MailPoetVendor\Symfony\Component\Validator\Mapping\Factory\MetadataFactoryInterface $metadataFactory, \MailPoetVendor\Symfony\Component\Validator\ConstraintValidatorFactoryInterface $validatorFactory, array $objectInitializers = [])
    {
        $this->context = $context;
        $this->defaultPropertyPath = $context->getPropertyPath();
        $this->defaultGroups = [$context->getGroup() ?: \MailPoetVendor\Symfony\Component\Validator\Constraint::DEFAULT_GROUP];
    ...
    public function validate($value, $constraints = null, $groups = null)
    {
        ...
        if ($this->context instanceof \MailPoetVendor\Symfony\Component\Validator\Context\ExecutionContext || \method_exists($this->context, 'getConstraint')) {
        ...
        if (null !== $constraints) {
            ...
            $metadata = new \MailPoetVendor\Symfony\Component\Validator\Mapping\GenericMetadata();
            $metadata->addConstraints($constraints);
            $this->validateGenericNode($value, $previousObject, \is_object($value) ? \spl_object_hash($value) : null, $metadata, $this->defaultPropertyPath, $groups, null, \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT, $this->context);
        ...
        if (\is_object($value)) {
            $this->validateObject($value, $this->defaultPropertyPath, $groups, \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT, $this->context);
            $this->context->setNode($previousValue, $previousObject, $previousMetadata, $previousPath);
            $this->context->setGroup($previousGroup);
            return $this;
        }
        ...
        throw new \MailPoetVendor\Symfony\Component\Validator\Exception\RuntimeException(\sprintf('Cannot validate values of type "%s" automatically. Please provide a constraint.', \gettype($value)));
    }
    /**
     * {@inheritdoc}
     */
    public function validateProperty($object, $propertyName, $groups = null)
    {
        ...
        if (!$classMetadata instanceof \MailPoetVendor\Symfony\Component\Validator\Mapping\ClassMetadataInterface) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ValidatorException(\sprintf('The metadata factory should return instances of "\\Symfony\\Component\\Validator\\Mapping\\ClassMetadataInterface", got: "%s".', \is_object($classMetadata) ? \get_class($classMetadata) : \gettype($classMetadata)));
        }
        $propertyMetadatas = $classMetadata->getPropertyMetadata($propertyName);
        $groups = $groups ? $this->normalizeGroups($groups) : $this->defaultGroups;
        $cacheKey = \spl_object_hash($object);
        $propertyPath = \MailPoetVendor\Symfony\Component\Validator\Util\PropertyPath::append($this->defaultPropertyPath, $propertyName);
        $previousValue = $this->context->getValue();
        $previousObject = $this->context->getObject();
        $previousMetadata = $this->context->getMetadata();
        $previousPath = $this->context->getPropertyPath();
        ...
        foreach ($propertyMetadatas as $propertyMetadata) {
            ...
            $this->validateGenericNode($propertyValue, $object, $cacheKey . ':' . \get_class($object) . ':' . $propertyName, $propertyMetadata, $propertyPath, $groups, null, \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT, $this->context);
        }
        $this->context->setNode($previousValue, $previousObject, $previousMetadata, $previousPath);
        $this->context->setGroup($previousGroup);
        return $this;
    ...
    public function validatePropertyValue($objectOrClass, $propertyName, $value, $groups = null)
    {
        ...
        $previousGroup = $this->context->getGroup();
        foreach ($propertyMetadatas as $propertyMetadata) {
            $this->validateGenericNode($value, $object, $cacheKey . ':' . $class . ':' . $propertyName, $propertyMetadata, $propertyPath, $groups, null, \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT, $this->context);
        }
        $this->context->setNode($previousValue, $previousObject, $previousMetadata, $previousPath);
        $this->context->setGroup($previousGroup);
        return $this;
     ...
     */
    private function validateObject($object, $propertyPath, array $groups, $traversalStrategy, \MailPoetVendor\Symfony\Component\Validator\Context\ExecutionContextInterface $context)
    {
        try {
            $classMetadata = $this->metadataFactory->getMetadataFor($object);
            if (!$classMetadata instanceof \MailPoetVendor\Symfony\Component\Validator\Mapping\ClassMetadataInterface) {
                throw new \MailPoetVendor\Symfony\Component\Validator\Exception\UnsupportedMetadataException(\sprintf('The metadata factory should return instances of "Symfony\\Component\\Validator\\Mapping\\ClassMetadataInterface", got: "%s".', \is_object($classMetadata) ? \get_class($classMetadata) : \gettype($classMetadata)));
            }
            $this->validateClassNode($object, \spl_object_hash($object), $classMetadata, $propertyPath, $groups, null, $traversalStrategy, $context);
        } catch (\MailPoetVendor\Symfony\Component\Validator\Exception\NoSuchMetadataException $e) {
            // Rethrow if not Traversable
            if (!$object instanceof \Traversable) {
                throw $e;
            }
            // Rethrow unless IMPLICIT or TRAVERSE
            if (!($traversalStrategy & (\MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT | \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::TRAVERSE))) {
                throw $e;
            }
     ...
     * @param ExecutionContextInterface $context      The current execution context
     */
    private function validateEachObjectIn($collection, $propertyPath, array $groups, \MailPoetVendor\Symfony\Component\Validator\Context\ExecutionContextInterface $context)
    {
        foreach ($collection as $key => $value) {
            ...
            if (\is_object($value)) {
                $this->validateObject($value, $propertyPath . '[' . $key . ']', $groups, \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT, $context);
            }
        }
    }
    /**
     ...
     */
    private function validateClassNode($object, $cacheKey, \MailPoetVendor\Symfony\Component\Validator\Mapping\ClassMetadataInterface $metadata = null, $propertyPath, array $groups, $cascadedGroups, $traversalStrategy, \MailPoetVendor\Symfony\Component\Validator\Context\ExecutionContextInterface $context)
    {
        ...
        foreach ($groups as $key => $group) {
            ...
            // every call.
            if (\MailPoetVendor\Symfony\Component\Validator\Constraint::DEFAULT_GROUP === $group) {
                if ($metadata->hasGroupSequence()) {
                    ...
                    $defaultOverridden = \true;
                    if (!$group instanceof \MailPoetVendor\Symfony\Component\Validator\Constraints\GroupSequence) {
                        $group = new \MailPoetVendor\Symfony\Component\Validator\Constraints\GroupSequence($group);
                    }
                }
            }
            ...
            if ($group instanceof \MailPoetVendor\Symfony\Component\Validator\Constraints\GroupSequence) {
                $this->stepThroughGroupSequence($object, $object, $cacheKey, $metadata, $propertyPath, $traversalStrategy, $group, $defaultOverridden ? \MailPoetVendor\Symfony\Component\Validator\Constraint::DEFAULT_GROUP : null, $context);
        ...
        foreach ($metadata->getConstrainedProperties() as $propertyName) {
            ...
            foreach ($metadata->getPropertyMetadata($propertyName) as $propertyMetadata) {
                if (!$propertyMetadata instanceof \MailPoetVendor\Symfony\Component\Validator\Mapping\PropertyMetadataInterface) {
                    throw new \MailPoetVendor\Symfony\Component\Validator\Exception\UnsupportedMetadataException(\sprintf('The property metadata instances should implement "Symfony\\Component\\Validator\\Mapping\\PropertyMetadataInterface", got: "%s".', \is_object($propertyMetadata) ? \get_class($propertyMetadata) : \gettype($propertyMetadata)));
                }
                $propertyValue = $propertyMetadata->getPropertyValue($object);
                $this->validateGenericNode($propertyValue, $object, $cacheKey . ':' . \get_class($object) . ':' . $propertyName, $propertyMetadata, \MailPoetVendor\Symfony\Component\Validator\Util\PropertyPath::append($propertyPath, $propertyName), $groups, $cascadedGroups, \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT, $context);
            }
        }
        // If no specific traversal strategy was requested when this method
        // was called, use the traversal strategy of the class' metadata
        if ($traversalStrategy & \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT) {
        ...
        if ($traversalStrategy & \MailPoetVendor\Symfony\Component\Validator\Mapping\TraversalStrategy::IMPLICIT && !$object instanceof \Traversable) {
            return;
        }
        // If TRAVERSE, fail if we have no Traversable
        if (!$object instanceof \Traversable) {
            throw new \MailPoetVendor\Symfony\Component\Validator\Exception\ConstraintDefinitionException(\sprintf('Traversal was enabled for "%s", but this class does not implement "\\Traversable".', \get_class($object)));
        }
        $this->validateEachObjectIn($object, $propertyPath, $groups, $context);
    }
    /**
     ...
     * @see TraversalStrategy
     */
    private function validateGenericNode($value, $object, $cacheKey, \MailPoetVendor\Symfony\Component\Validator\Mapping\MetadataInterface $metadata = null, $propertyPath, array $groups, $cascadedGroups, $traversalStrategy, \MailPoetVendor\Symfony\Component\Validator\Context\ExecutionContextInterface $context)
    {
        ...
        if (!($cascadingStrategy & \MailPoetVendor\Symfony\Component\Validator\Mapping\CascadingStrategy::CASCADE)) {
            return;
        }
        // If no specific traversal strategy was requested when this method
     ...
     * @param ExecutionContextInterface $context           The execution context
     */
    private function stepThroughGroupSequence($value, $object, $cacheKey, \MailPoetVendor\Symfony\Component\Validator\Mapping\MetadataInterface $metadata = null, $propertyPath, $traversalStrategy, \MailPoetVendor\Symfony\Component\Validator\Constraints\GroupSequence $groupSequence, $cascadedGroup, \MailPoetVendor\Symfony\Component\Validator\Context\ExecutionContextInterface $context)
    {
        $violationCount = \count($context->getViolations());
        $cascadedGroups = $cascadedGroup ? [$cascadedGroup] : null;
        foreach ($groupSequence->groups as $groupInSequence) {
            $groups = (array) $groupInSequence;
            if ($metadata instanceof \MailPoetVendor\Symfony\Component\Validator\Mapping\ClassMetadataInterface) {
     ...
     * @param ExecutionContextInterface $context  The execution context
     */
    private function validateInGroup($value, $cacheKey, \MailPoetVendor\Symfony\Component\Validator\Mapping\MetadataInterface $metadata, $group, \MailPoetVendor\Symfony\Component\Validator\Context\ExecutionContextInterface $context)
    {
        ...
        foreach ($metadata->findConstraints($group) as $constraint) {
            ...
            if (null !== $cacheKey) {
                ...
                if ($constraint instanceof \MailPoetVendor\Symfony\Component\Validator\Constraints\Composite) {
